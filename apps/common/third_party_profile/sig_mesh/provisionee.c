...
#define LOG_TAG             "[MESH-provisionee]"
...
#pragma bss_seg(".ble_mesh_provisionee_bss")
#pragma data_seg(".ble_mesh_provisionee_data")
#pragma const_seg(".ble_mesh_provisionee_const")
#pragma code_seg(".ble_mesh_provisionee_code")
#endif
...
static void prov_invite(const u8_t *data)
{
    ...
    u16_t algorithm_bm = 0;
    u8_t oob_type = bt_mesh_prov->static_val ?
...
static void send_confirm(void)
{
    ...
    llsync_mesh_auth_clac(bt_mesh_prov_link.rand, bt_mesh_prov_link.auth);
#endif
...
{
    const u8_t *remote_pub_key;
    const u8_t *remote_priv_key;
     ...
     */
    sys_memcpy_swap(remote_pk_le, remote_pub_key, 32);
    sys_memcpy_swap(&remote_pk_le[32], &remote_pub_key[32], 32);
...
#if (CONFIG_MESH_MODEL == SIG_MESH_TENCENT_MESH)
    llsync_mesh_auth_clac(data, bt_mesh_prov_link.auth);
#endif
...
bool is_pb_adv(void)
{
    if (!bt_mesh_prov_link.bearer) {
        return 0;
    }

    return (bt_mesh_prov_link.bearer->type == BT_MESH_PROV_ADV);
}

static bool refresh_is_valid(const u8_t *netkey, u16_t net_idx,
                             u32_t iv_index)
{
    enum bt_mesh_rpr_node_refresh proc = bt_mesh_node_refresh_get();
    struct bt_mesh_subnet *sub = bt_mesh_subnet_get(net_idx);
    u16_t old_addr = bt_mesh_primary_addr();
    bool valid_addr;
...
static void prov_data(const u8_t *data)
{
     ...
     * using Node Identity.
     */
    LOG_DBG("identity_enable 0x%x", identity_enable);
