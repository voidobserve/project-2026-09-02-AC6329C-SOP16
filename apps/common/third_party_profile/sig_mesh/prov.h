...
struct bt_mesh_prov_role {
    ...
    void (*error)(u8_t reason);

    void (*input_complete)(void);

    void (*op[10])(const u8_t *data);
};
...
struct bt_mesh_prov_link {
    ...
    u16_t addr;                        /* Assigned address */

    u8_t algorithm;                    /* Authen algorithm */
    u8_t oob_method;                   /* Authen method */
    u8_t oob_action;                   /* Authen action */
    u8_t oob_size;                     /* Authen size */
    u8_t auth[PROV_AUTH_MAX_LEN];      /* Authen value */

    u8_t dhkey[DH_KEY_SIZE];	          /* Calculated DHKey */
    u8_t expect;                       /* Next expected PDU */

    u8_t conf[PROV_AUTH_MAX_LEN];      /* Local/Remote Confirmation */
    ...
    struct {
        u8_t invite[PDU_LEN_INVITE];
        u8_t capabilities[PDU_LEN_CAPABILITIES];
        u8_t start[PDU_LEN_START];
        u8_t pub_key_provisioner[PDU_LEN_PUB_KEY]; /* big-endian */
        u8_t pub_key_device[PDU_LEN_PUB_KEY]; /* big-endian */
    } conf_inputs;
...
static inline void bt_mesh_prov_buf_init(struct net_buf_simple *buf, u8_t type)
{
...
static inline u8_t bt_mesh_prov_auth_size_get(void)
{
    return bt_mesh_prov_link.algorithm == BT_MESH_PROV_AUTH_CMAC_AES128_AES_CCM ? 16 : 32;
}

static inline u32_t bt_mesh_prov_protocol_timeout_get(void)
{
...
int bt_mesh_prov_auth(bool is_provisioner, u8_t method, u8_t action, u8_t size);
...
int bt_mesh_pb_remote_open_node(struct bt_mesh_rpr_cli *cli,
                                ...
                                u16_t addr, bool composition_change);
