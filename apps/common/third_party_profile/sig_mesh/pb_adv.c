...
#pragma bss_seg(".ble_mesh_pbadv_bss")
#pragma data_seg(".ble_mesh_pbadv_data")
#pragma const_seg(".ble_mesh_pbadv_const")
#pragma code_seg(".ble_mesh_pbadv_code")
#endif
...
static void delayed_adv_send_start(u16_t duration, int err, void *user_data)
{
    if (err) {
...
static struct bt_mesh_adv *adv_create(u8_t retransmits)
{
    struct bt_mesh_adv *adv;
...
{
    LOG_DBG("xact 0x%x complete", (u8_t)link.tx.pending_ack);
...
static void gen_prov_cont(struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    /* not used to check tx */
    /*if (link.tx.adv[0]) {
    	LOG_ERR(">> %s Ongoing tx transaction has not been completed yet", __func__);
    	return;
    }*/

    LOG_DBG(">> %s len %u, seg_index %u", __func__, buf->len, seg);
    ...
    if (!link.rx.seg && link.rx.id == rx->xact_id) {
        ...
        LOG_WRN("No seg and rx id collision");
        return;
    }
...
{
    LOG_DBG(">> %s len %u", __func__, buf->len);
...
static struct bt_mesh_adv *ctl_adv_create(u8_t op, const void *data, u8_t data_len,
        u8_t retransmits)
{
    struct bt_mesh_adv *adv;
...
void bt_mesh_pb_adv_recv(struct net_buf_simple *buf)
{
    ...
    if (atomic_test_bit(link.flags, ADV_LINK_ACTIVE) && link.id != rx.link_id) {
        LOG_WRN("link active and id diff.");
        return;
    }
...
static int prov_link_open(const u8_t uuid[16], u8_t timeout,
