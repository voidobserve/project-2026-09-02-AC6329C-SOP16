...
static struct seg_tx {
    ...
    u64_t              seq_auth;
    s64_t               adv_start_timestamp; /* Calculate adv duration and adjust intervals*/
    u16_t              src;
    u16_t              dst;
    u16_t              ack_src;
    u16_t              len;
    ...
    u8_t               attempts_left;
    u8_t               attempts_left_without_progress;
...
// K_MEM_SLAB_DEFINE(segs, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS, 4);//for compiler, not used now.
...
{
    // k_mem_slab_free(&segs, (void *)tx->seg[seg_idx]);//for compiler, not used now.
    free((void *)tx->seg[seg_idx]);
...
static void seg_tx_unblock_check(struct seg_tx *tx)
{
    ...
    if (blocked) {
        LOG_DBG("Unblocked 0x%04x", (u16_t)(blocked->seq_auth & TRANS_SEQ_ZERO_MASK));
...
static void schedule_transmit_continue(struct seg_tx *tx, u32_t delta)
{
    u32_t timeout = 0;

    if (!tx->nack_count) {
        LOG_ERR(">>> %s tx->nack_count %d ", __func__, tx->nack_count);
        return;
    }
    ...
    LOG_DBG("timeout %d seg_o 0x%x  seg_n 0x%x ", timeout, tx->seg_o, tx->seg_n);
...
{
    struct seg_tx *tx = user_data;
    u32_t delta_ms = (u32_t)(k_uptime_get() - tx->adv_start_timestamp);
...
static void seg_tx_send_unacked(struct seg_tx *tx)
{
    ...
    LOG_DBG("SeqZero: 0x%04x Attempts: %u",
            (u16_t)(tx->seq_auth & TRANS_SEQ_ZERO_MASK), tx->attempts_left);
    ...
    delta_ms = (u32_t)(k_uptime_get() - tx->adv_start_timestamp);
    if (tx->ack_received) {
                    ...
                    u8_t *ctl_op)
{
    ...
    LOG_DBG("SeqZero 0x%04x (segs: %u)", (u16_t)(tx->seq_auth & TRANS_SEQ_ZERO_MASK),
            tx->nack_count);
    ...
    for (seg_o = 0U; sdu->len; seg_o++) {
        ...
        // err = k_mem_slab_alloc(&segs, &buf, BUF_TIMEOUT);//for compiler, not used now.
        buf = malloc(BT_MESH_APP_SEG_SDU_MAX);
        ...
        if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
            ...
            if (bt_mesh_friend_enqueue_tx(
                 ...
                 * out through the Friend Queue.
                 */
                // k_mem_slab_free(&segs, buf);//for compiler, not used now.
                free(buf);
                     ...
                     struct net_buf_simple *buf, u64_t *seq_auth)
{
    ...
    *seq_auth = tx->seq_auth;
    ...
    while ((bit = find_lsb_set(ack))) {
        ...
        ack &= ~BIT(bit - 1);
        LOG_DBG("0x%x 0x%x 0x%x", bit, ack, find_lsb_set(ack));
    }
    ...
    if (tx->nack_count) {
         ...
         */
        LOG_DBG("%s L %d tx->seg_o 0x%x", __func__, __LINE__, tx->seg_o);
        if (tx->seg_o == 0) {
...
reschedule:
            LOG_DBG("%s L %d timeout %d", __func__, __LINE__, timeout);
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    LOG_DBG("len %u: %s", data_len, bt_hex(data, data_len));
...
static void seg_rx_reset(struct seg_rx *rx, bool full_reset)
{
    ...
    for (i = 0; i <= rx->seg_n; i++) {
        ...
        // k_mem_slab_free(&segs, rx->seg[i]);//for compiler, not used now.
        free(rx->seg[i]);
                                   ...
                                   u8_t seg_n)
{
     ...
     */
    // if (k_mem_slab_num_free_get(&segs) < 1) {	//for compiler, not used now.
...
found_rx:
    ...
    // err = k_mem_slab_alloc(&segs, &rx->seg[seg_o], K_NO_WAIT);//for compiler, not used now.
    rx->seg[seg_o] = malloc(BT_MESH_APP_SEG_SDU_MAX);
