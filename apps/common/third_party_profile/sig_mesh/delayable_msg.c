...
#define LOG_TAG             "[MESH-delayablemsg]"
...
#pragma bss_seg(".ble_mesh_delayablemsg_bss")
#pragma data_seg(".ble_mesh_delayablemsg_data")
#pragma const_seg(".ble_mesh_delayablemsg_const")
#pragma code_seg(".ble_mesh_delayablemsg_code")
#endif
...
static struct delayable_msg_chunk {
    sys_snode_t node;
    u8_t data[CONFIG_BT_MESH_ACCESS_DELAYABLE_MSG_CHUNK_SIZE];
...
static bool push_msg_from_delayable_msgs(void)
{
    ...
    SYS_SLIST_FOR_EACH_NODE(&msg->chunks, node) {
        u16_t tmp = MIN(CONFIG_BT_MESH_ACCESS_DELAYABLE_MSG_CHUNK_SIZE, len);
...
int bt_mesh_delayable_msg_manage(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf,
                                 u16_t src_addr, const struct bt_mesh_send_cb *cb, void *cb_data)
{
    sys_snode_t *node;
    struct delayable_msg_ctx *msg;
    u16_t random_delay;
    ...
    SYS_SLIST_FOR_EACH_NODE(&msg->chunks, node) {
        u16_t tmp = MIN(CONFIG_BT_MESH_ACCESS_DELAYABLE_MSG_CHUNK_SIZE, buf->len);
    ...
    bt_rand(&random_delay, sizeof(u16_t));
