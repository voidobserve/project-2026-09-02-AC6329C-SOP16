...
#pragma bss_seg(".ble_mesh_rprcli_bss")
#pragma data_seg(".ble_mesh_rprcli_data")
#pragma const_seg(".ble_mesh_rprcli_const")
#pragma code_seg(".ble_mesh_rprcli_code")
#endif
...
static s32_t tx_timeout = (2 * MSEC_PER_SEC);
                              ...
                              struct net_buf_simple *buf)
{
    ...
    struct bt_mesh_rpr_cli *cli = mod->rt->user_data;
    struct bt_mesh_rpr_link link;
    u8_t reason = PROV_BEARER_LINK_STATUS_SUCCESS;
...
static void pdu_send_start(u16_t duration, int err, void *cb_data)
{
...
static int tx_wait(struct bt_mesh_rpr_cli *cli,
                   ...
                   struct net_buf_simple *buf, u32_t rsp, void *rsp_ctx)
{
...
int bt_mesh_rpr_scan_start(struct bt_mesh_rpr_cli *cli,
                           const struct bt_mesh_rpr_node *srv,
                           const u8_t uuid[16], u8_t timeout,
                           u8_t max_devs,
                           struct bt_mesh_rpr_scan_status *status)
{
    if (!timeout) {
...
int bt_mesh_rpr_scan_start_ext(struct bt_mesh_rpr_cli *cli,
                               const struct bt_mesh_rpr_node *srv,
                               const u8_t uuid[16], u8_t timeout,
                               const u8_t *ad_types, size_t ad_count)
{
    struct bt_mesh_msg_ctx ctx = LINK_CTX(srv, false);
...
s32_t bt_mesh_rpr_cli_timeout_get(void)
{
    return tx_timeout;
}

void bt_mesh_rpr_cli_timeout_set(s32_t timeout)
{
    tx_timeout = timeout;
}
...
static int pb_link_open(const u8_t uuid[16], u8_t timeout,
