...
#pragma bss_seg(".ble_mesh_proxysrv_bss")
#pragma data_seg(".ble_mesh_proxysrv_data")
#pragma const_seg(".ble_mesh_proxysrv_const")
#pragma code_seg(".ble_mesh_proxysrv_code")
#endif
...
static void identity_enabled(struct bt_mesh_subnet *sub)
{
    ...
    // STRUCT_SECTION_FOREACH(bt_mesh_proxy_cb, cb) {	//for compiler, not used now.
...
static int enc_id_adv(struct bt_mesh_subnet *sub, u8_t type,
                      u8_t hash[16], s32_t duration)
{
    int err;
    u8_t *proxy_svc_data = get_server_data_addr();
...
static int node_id_adv(struct bt_mesh_subnet *sub, s32_t duration)
{
    u8_t *proxy_svc_data = get_server_data_addr();
    u8_t *random = &proxy_svc_data[11];
    u8_t tmp[16];
    int err;
...
static int priv_node_id_adv(struct bt_mesh_subnet *sub, s32_t duration)
{
    u8_t *proxy_svc_data = get_server_data_addr();
    u8_t *random = &proxy_svc_data[11];
    u8_t tmp[16];
...
static int priv_net_id_adv(struct bt_mesh_subnet *sub, s32_t duration)
{
    u8_t *proxy_svc_data = get_server_data_addr();
    u8_t *random = &proxy_svc_data[11];
    u8_t tmp[16];
...
static int net_id_adv(struct bt_mesh_subnet *sub, s32_t duration)
{
    u8_t *proxy_svc_data = get_server_data_addr();
...
static bool proxy_adv_request_get(struct bt_mesh_subnet *sub, struct proxy_adv_request *request)
{
    ...
    if (bt_mesh_od_priv_proxy_get() > 0 && sub->solicited) {
        s32_t timeout = MSEC_PER_SEC * (s32_t)bt_mesh_od_priv_proxy_get();
...
static int gatt_proxy_advertise(void)
{
    ...
    if (!cnt) {
        ...
        if ((sub_adv.request.duration != SYS_FOREVER_MS) &&
            ...
            s32_t time_passed = k_uptime_get_32() - sub_adv.start;
...
static u32_t svc_reg_attempts;

static void svc_reg_work_handler(struct k_work *work)
{
    // int err; //for compiler, not used now.

    // err =
    bt_gatt_service_register(BT_UUID_MESH_PROXY_VAL);
    // if ((err == -EINVAL) && ((--svc_reg_attempts) > 0)) { //for compiler, not used now.
...
int bt_mesh_proxy_gatt_enable(void)
{
    ...
    k_work_schedule(&svc_reg_work, PROXY_SVC_INIT_TIMEOUT);
...
{
    LOG_DBG("bt_mesh_proxy_gatt_disable");
    ...
    bt_gatt_service_unregister(BT_UUID_MESH_PROXY_VAL);
...
void proxy_srv_connected(struct bt_conn *conn, u8_t conn_err)
{
    struct bt_mesh_proxy_client *client;

    if (!service_registered) {
        //LOG_ERR("gatt_connected err!");
        return;
    }

    LOG_DBG("gatt_connected conn %p err 0x%02x", (void *)conn, conn_err);
...
void proxy_srv_disconnected(struct bt_conn *conn, u8_t reason)
{
    struct bt_mesh_proxy_client *client;

    LOG_DBG("gatt_disconnected conn %p reason 0x%02x", (void *)conn, reason);
...
u8_t bt_mesh_proxy_srv_connected_cnt(void)
{
    u8_t cnt = 0;
