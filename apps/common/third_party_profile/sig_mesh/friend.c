...
#if 0 //NET_BUF_USE_MALLOC for friend
struct friend_adv {
    u16_t app_idx;
    struct {
...
static struct friend_adv *adv_pool;
#else
struct friend_adv {
    u16_t app_idx;
    struct {
...
static struct friend_adv adv_pool[5];//FRIEND_BUF_COUNT
...
static int friend_cred_create(struct bt_mesh_friend *frnd, u8_t idx)
{
...
static void friend_clear(struct bt_mesh_friend *frnd)
{
    ...
    // STRUCT_SECTION_FOREACH(bt_mesh_friend_cb, cb) {	//for compiler, not used now.
        ...
        struct net_buf_simple *sdu)
{
    ...
    // buf = net_buf_alloc(&friend_buf_pool, K_NO_WAIT);	//for compiler, not used now.
    if (!buf) {
        return NULL;
    }
                                ...
                                struct unseg_app_sdu_meta *meta)
{
    ...
    u16_t uuidx = FRIEND_ADV(buf)->uuidx;
...
static bool friend_queue_check_dup(struct bt_mesh_friend *frnd, u32_t seq,
                                   u16_t src)
{
    struct bt_mesh_net_rx rx;
    sys_snode_t *cur;
...
int bt_mesh_friend_terminate(u16_t lpn_addr)
{
    struct bt_mesh_friend *frnd;
...
{
    LOG_DBG("--func=%s, adv buffer cnt = %d", __FUNCTION__, FRIEND_BUF_COUNT);
    ...
    LOG_DBG("net_buf_data size=0x%x", ALIGN_4BYTE(FRIEND_BUF_COUNT * BT_MESH_ADV_DATA_SIZE));
    adv_pool_p = buf_size;
    buf_size += (sizeof(struct friend_adv) * FRIEND_BUF_COUNT);
    LOG_DBG("adv_pool size=0x%x", sizeof(struct friend_adv) * FRIEND_BUF_COUNT);
    frnd_p = buf_size;
    buf_size += (sizeof(struct bt_mesh_friend) * CONFIG_BT_MESH_FRIEND_LPN_COUNT);
    LOG_DBG("frnd size=0x%x", sizeof(struct bt_mesh_friend) * CONFIG_BT_MESH_FRIEND_LPN_COUNT);
    sub_list_p = buf_size;
    buf_size += ALIGN_4BYTE(FRIEND_SUB_LIST_SIZE * sizeof(u16_t));
    LOG_DBG("sub_list size=0x%x", ALIGN_4BYTE(FRIEND_SUB_LIST_SIZE * sizeof(u16_t)));
    seg_p = buf_size;
    buf_size += (sizeof(struct bt_mesh_friend_seg) * FRIEND_SEG_RX);
    LOG_DBG("seg size=0x%x", sizeof(struct bt_mesh_friend_seg) * FRIEND_SEG_RX);
    frnd_cred_p = buf_size;
    buf_size += bt_mesh_friend_cred_size_need();
    LOG_DBG("frnd_cred size=0x%x", bt_mesh_friend_cred_size_need());
    ...
    LOG_DBG("frnd addr=0x%x", frnd_p);
    LOG_DBG("sub_list addr=0x%x", sub_list_p);
    LOG_DBG("seg addr=0x%x", seg_p);
    LOG_DBG("frnd_cred addr=0x%x", frnd_cred_p);
    LOG_DBG("*fixed->data_pool=0x%x", *((u32 *)net_buf_fixed_friend_buf_pool.data_pool));
