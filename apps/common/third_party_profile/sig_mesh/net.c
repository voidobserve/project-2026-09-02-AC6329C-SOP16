...
struct loopback_buf {
    ...
    u8_t len;
    u8_t data[LOOPBACK_MAX_PDU_LEN];
};
...
int bt_mesh_net_create(u16_t idx, u8_t flags, const struct bt_mesh_key *key,
                       u32_t iv_index)
{
    int err;
...
static void bt_mesh_net_local(struct k_work *work)
{
    ...
    while ((node = sys_slist_get(&bt_mesh.local_queue))) {
        ...
        // k_mem_slab_free(&loopback_buf_pool, (void *)buf);//for compiler, not used now.
    }
}
...
static int net_encrypt(struct net_buf_simple *buf,
                       const struct bt_mesh_net_cred *cred, u32_t iv_index,
...
static struct loopback_buf *loopback_buffuer = NULL;

static int net_loopback(const struct bt_mesh_net_tx *tx, const u8_t *data,
                        size_t len)
{
    int err;
    // struct loopback_buf *buf;
    loopback_buffuer = malloc(sizeof(struct loopback_buf));
    if (err) {
        LOG_WRN("Unable to allocate loopback");
        return -ENOMEM;
    }
    loopback_buffuer->sub = tx->sub;
    (void)memcpy(loopback_buffuer->data, data, len);
    loopback_buffuer->len = len;
    sys_slist_append(&bt_mesh.local_queue, &loopback_buffuer->node);
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY)) {
        if (bt_mesh_proxy_relay(adv, tx->ctx->addr) &&
            BT_MESH_ADDR_IS_UNICAST(tx->ctx->addr)) {
            /* Notify completion if this only went
             * through the Mesh Proxy.
             * Don't use bt_mesh_adv_send().
             */
            err = 0;
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_PROXY_CLIENT)) {
        if (bt_mesh_proxy_cli_relay(adv)) {
            /* Notify completion if this only went
             * through the Mesh Proxy Client.
             * Don't use bt_mesh_adv_send().
             */
            err = 0;
...
void bt_mesh_net_loopback_clear(u16_t net_idx)
{
    ...
    while ((node = sys_slist_get(&bt_mesh.local_queue))) {
        ...
        if (net_idx == BT_MESH_KEY_ANY || net_idx == loopback_buffuer->sub->net_idx) {
            // LOG_DBG("Dropped 0x%06x", SEQ(buf->data));
            free(loopback_buffuer);
            loopback_buffuer = NULL;
            // k_mem_slab_free(&loopback_buf_pool, (void *)buf);//for compiler, not used now.
        } else {
            sys_slist_append(&new_list, &loopback_buffuer->node);
        }
    }
...
#if (CONFIG_BT_MESH_DFU_TARGET) || (CONFIG_BT_MESH_DFU_DIST)
    /* limit message relay for mesh dfu.--yuhang*/
    static uint8_t relay_limit = 0;
    if (relay_limit < 200) {
        relay_limit++;
    } else {
        return;
