...
static void (*msg_cb)(u32_t opcode, struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf);
...
static u8_t *data_buf_add_u8_offset(struct net_buf_simple *buf,
                                    u8_t val, size_t *offset)
{
    if (*offset >= 1) {
        *offset -= 1;
        return NULL;
...
static void data_buf_add_mem_offset(struct net_buf_simple *buf, const u8_t *data, size_t len,
                                    size_t *offset)
{
    if (*offset >= len) {
        *offset -= len;
...
static uint8_t count_mod_ext(const struct bt_mesh_model *mod,
                             u8_t *max_offset, u8_t sig_offset)
{
    int i;
    u8_t extensions = 0;
    s8_t offset, offset_record = 0;
...
static bool is_cor_present(const struct bt_mesh_model *mod, u8_t *cor_id, u8_t sig_offset)
{
    int i;
...
static void prep_model_item_header(const struct bt_mesh_model *mod, u8_t *cor_id,
                                   u8_t *mod_cnt, struct net_buf_simple *buf,
                                   size_t *offset, u8_t sig_offset)
{
    u8_t ext_mod_cnt;
    bool cor_present;
    u8_t mod_elem_info = 0;
    s8_t max_offset;
...
static void add_items_to_page(struct net_buf_simple *buf, const struct bt_mesh_model *mod,
                              u8_t ext_mod_cnt, size_t *offset, u8_t sig_offset)
{
    int i, elem_offset;
    u8_t mod_idx;
...
static size_t mod_items_size(const struct bt_mesh_model *mod, u8_t sig_offset)
{
    int i, offset;
    size_t temp_size = 0;
...
static s32_t next_period(const struct bt_mesh_model *mod)
{
...
static u16_t pub_delay_get(int random_delay_window)
{
...
void bt_mesh_comp_provision(u16_t addr)
{
    ...
    LOG_DBG("addr 0x%04x elem_count %u", addr, dev_comp->elem_count);
...
static u16_t *model_group_get(const struct bt_mesh_model *mod, u16_t addr)
{
    int i;
...
static const u8_t **model_uuid_get(const struct bt_mesh_model *mod, const u8_t *uuid)
{
    int i;
...
static bool model_has_dst(const struct bt_mesh_model *mod, u16_t dst, const u8_t *uuid)
{
...
static const struct bt_mesh_model_op *find_op(const struct bt_mesh_elem *elem,
        u32_t opcode, const struct bt_mesh_model **model)
{
    u8_t i;
    u8_t count;
    /* This value shall not be used in shipping end products. */
    u32_t cid = UINT32_MAX;
...
static int element_model_recv(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf,
                              const struct bt_mesh_elem *elem, u32_t opcode)
{
    const struct bt_mesh_model_op *op;
    const struct bt_mesh_model *model;
    ...
    if (!bt_mesh_model_has_key(model, ctx->app_idx)) {
        LOG_ERR("Model at 0x%04x is not bound to app idx %d", elem->rt->addr, ctx->app_idx);
        return ACCESS_STATUS_WRONG_KEY;
    }

    if (!model_has_dst(model, ctx->recv_dst, ctx->uuid)) {
        LOG_ERR("Dst addr 0x%02x is invalid for model at 0x%04x", ctx->recv_dst,
                elem->rt->addr);
...
void bt_mesh_model_extensions_walk(const struct bt_mesh_model *model,
                                   ...
                                   void *user_data)
{
#if (CONFIG_BT_MESH_MODEL_EXTENSIONS != 1)
                             ...
                             const struct bt_mesh_model *base_mod)
{
    ...
    u8_t base_offset = get_sig_offset(base_mod);
    u8_t corresponding_offset = get_sig_offset(corresponding_mod);
...
int bt_mesh_comp_store(void)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(comp_data_pages); i++) {
        ...
        node_info_store(comp_data_pages[i].path, buf.data, buf.len);
...
static void comp_data_clear(void)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(comp_data_pages); i++) {
        node_info_clear(comp_data_pages[i].path, bt_mesh_comp_page_size(i));
        if (err) {
...
int bt_mesh_comp_read(struct net_buf_simple *buf, uint8_t page)
{
    ...
    err = node_info_load(comp_data_pages[i].path, buf, bt_mesh_comp_page_size(i));
...
int bt_mesh_models_metadata_store(void)
{
    ...
    node_info_store(METADATA_PAGE_INDEX, buf.data, buf.len);
...
int bt_mesh_models_metadata_read(struct net_buf_simple *buf, size_t offset)
{
    ...
    err = node_info_load(METADATA_PAGE_INDEX, &stored_buf, stored_buf.len);
    if (err) {
...
static void models_metadata_clear(void)
{
    ...
    node_info_clear(METADATA_PAGE_INDEX, bt_mesh_metadata_page_0_size());
...
{
    u8_t page = net_buf_simple_pull_u8(buf);
