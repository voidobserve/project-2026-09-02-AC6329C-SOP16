...
#pragma bss_seg(".ble_mesh_heartbeat_bss")
#pragma data_seg(".ble_mesh_heartbeat_data")
#pragma const_seg(".ble_mesh_heartbeat_const")
#pragma code_seg(".ble_mesh_heartbeat_code")
#endif
...
{
    // STRUCT_SECTION_FOREACH(bt_mesh_hb_cb, cb) {	//for compiler, not used now.
    // 	if (cb->pub_sent) {
    // 		cb->pub_sent(&pub);
    // 	}
    // }
...
static int64_t sub_remaining(void)
{
    ...
    // u32_t rem_ms = k_ticks_to_ms_floor32(
    // 	k_work_delayable_remaining_get(&sub_timer));//for compiler, not used now.
    u32_t rem_ms = 1000;
...
static void notify_recv(u8_t hops, u16_t feat)
{
...
u8_t bt_mesh_hb_pub_set(struct bt_mesh_hb_pub *new_pub)
{
