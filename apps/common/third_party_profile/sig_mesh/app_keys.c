...
#pragma bss_seg(".ble_mesh_appkeys_bss")
#pragma data_seg(".ble_mesh_appkeys_data")
#pragma const_seg(".ble_mesh_appkeys_const")
#pragma code_seg(".ble_mesh_appkeys_code")
#endif
...
struct app_key *app_get(u16_t app_idx)
{
...
static struct app_key_update *app_key_update_find(u16_t key_idx,
...
static void update_app_key_settings(u16_t app_idx, bool store)
{
...
{
    // STRUCT_SECTION_FOREACH(bt_mesh_app_key_cb, cb) {	//for compiler, not used now.
    // 	cb->evt_handler(app->app_idx, app->net_idx, evt);
    // }
}

static struct app_key *app_key_alloc(u16_t app_idx)
{
...
u8_t bt_mesh_app_key_add(u16_t app_idx, u16_t net_idx,
                         const u8_t key[16])
{
    struct app_key *app;
...
u8_t bt_mesh_app_key_update(u16_t app_idx, u16_t net_idx,
                            const u8_t key[16])
{
    struct app_key *app;
...
u8_t bt_mesh_app_key_del(u16_t app_idx, u16_t net_idx)
{
    struct app_key *app;
...
int bt_mesh_app_key_set(u16_t app_idx, u16_t net_idx,
...
bool bt_mesh_app_key_exists(u16_t app_idx)
{
    for (int i = 0; i < ARRAY_SIZE(apps); i++) {
        if (apps[i].app_idx == app_idx) {
            return true;
...
ssize_t bt_mesh_app_keys_get(u16_t net_idx, u16_t app_idxs[], size_t max,
                             off_t skip)
{
    size_t count = 0;
...
int bt_mesh_keys_resolve(struct bt_mesh_msg_ctx *ctx,
                         struct bt_mesh_subnet **sub,
                         const struct bt_mesh_key **app_key, u8_t *aid)
{
    struct app_key *app = NULL;
...
u16_t bt_mesh_app_key_find(bool dev_key, u8_t aid,
