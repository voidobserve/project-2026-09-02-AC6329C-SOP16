...
#pragma bss_seg(".ble_mesh_dfdcli_bss")
#pragma data_seg(".ble_mesh_dfdcli_data")
#pragma const_seg(".ble_mesh_dfdcli_const")
#pragma code_seg(".ble_mesh_dfdcli_code")
#endif
...
static s32_t dfu_cli_timeout = (10 * MSEC_PER_SEC);
...
static int req_setup(struct bt_mesh_dfu_cli *cli, enum req type, u16_t addr,
                     void *params)
{
...
static int req_wait(struct bt_mesh_dfu_cli *cli, k_timeout_t timeout)
{
    ...
    err = os_sem_pend(&cli->req.sem, timeout);
...
static void tx_start(u16_t dur, int err, void *cb_data);
...
static void send_info_get(struct bt_mesh_blob_cli *b, u16_t dst)
{
    struct bt_mesh_dfu_cli *cli = DFU_CLI(b);
...
static void send_update_start(struct bt_mesh_blob_cli *b, u16_t dst)
{
    struct bt_mesh_dfu_cli *cli = DFU_CLI(b);
    struct bt_mesh_msg_ctx ctx = MSG_CTX(cli, dst);
    struct bt_mesh_dfu_target *target;
...
static void send_update_get(struct bt_mesh_blob_cli *b, u16_t dst)
{
    struct bt_mesh_dfu_cli *cli = DFU_CLI(b);
    struct bt_mesh_msg_ctx ctx = MSG_CTX(cli, dst);
...
static void send_update_cancel(struct bt_mesh_blob_cli *b, u16_t dst)
{
    struct bt_mesh_dfu_cli *cli = DFU_CLI(b);
    struct bt_mesh_msg_ctx ctx = MSG_CTX(cli, dst);
...
static void send_update_apply(struct bt_mesh_blob_cli *b, u16_t dst)
{
    struct bt_mesh_dfu_cli *cli = DFU_CLI(b);
    struct bt_mesh_msg_ctx ctx = MSG_CTX(cli, dst);
        ...
        void *cb_data)
{
    ...
    if (target) {
        LOG_INF("SUCCESS: 0x%04x applied dfu (as image %u)", ctx->addr,
                idx);
        target->phase = BT_MESH_DFU_PHASE_APPLY_SUCCESS;
        blob_cli_broadcast_rsp(&cli->blob, &target->blob);
        notify_dfu_target_state(ctx->addr, target->phase, target->status);
    } else {
...
static void confirmed(struct bt_mesh_blob_cli *b)
{
    ...
    TARGETS_FOR_EACH(cli, target) {
        ...
        if (target->effect == BT_MESH_DFU_EFFECT_UNPROV) {
            ...
            LOG_DBG(">>> %s Target 0x%04x still provisioned", __func__, target->blob.addr);
            target->phase = BT_MESH_DFU_PHASE_APPLY_FAIL;
            target_failed(cli, target, BT_MESH_DFU_ERR_INTERNAL);
        } else if (!target->blob.acked) {
            LOG_DBG(">>> %s Target 0x%04x failed to respond", __func__, target->blob.addr);
            target->phase = BT_MESH_DFU_PHASE_APPLY_FAIL;
            target_failed(cli, target, BT_MESH_DFU_ERR_INTERNAL);
                         ...
                         struct net_buf_simple *buf)
{
    ...
    LOG_INF(">>> %s %u phase: %u, cur state: %u", __func__, status, phase, cli->xfer.state);
    ...
    if (buf->len == 13) {
        ...
        LOG_INF("Target 0x%04x receiving transfer", ctx->addr);
    } else if (buf->len) {
        return -EINVAL;
    }
                              ...
                              struct net_buf_simple *buf)
{
    ...
    LOG_INF(">>> %s Image list from 0x%04x from index %u (%u %u)", __func__, ctx->addr, idx,
            buf->len, cli->req.img_cnt);
...
u8_t bt_mesh_dfu_cli_progress(struct bt_mesh_dfu_cli *cli)
{
    if (cli->xfer.state == STATE_TRANSFER) {
...
int bt_mesh_dfu_cli_metadata_check(struct bt_mesh_dfu_cli *cli,
                                   struct bt_mesh_msg_ctx *ctx, u8_t img_idx,
...
s32_t bt_mesh_dfu_cli_timeout_get(void)
{
    return dfu_cli_timeout;
}

void bt_mesh_dfu_cli_timeout_set(s32_t t)
{
    dfu_cli_timeout = t;
}
