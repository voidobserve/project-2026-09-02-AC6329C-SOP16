...
#pragma bss_seg(".ble_mesh_dfdsrv_bss")
#pragma data_seg(".ble_mesh_dfdsrv_data")
#pragma const_seg(".ble_mesh_dfdsrv_const")
#pragma code_seg(".ble_mesh_dfdsrv_code")
#endif
...
static inline int set_upload_fwid(struct bt_mesh_dfd_srv *srv, struct bt_mesh_msg_ctx *ctx,
                                  const u8_t *fwid, size_t fwid_len)
{
                                   ...
                                   struct net_buf_simple *buf)
{
    ...
    LOG_HEXDUMP_INF(uri, uri_len);
    LOG_HEXDUMP_INF(fwid, fwid_len);
...
static void fw_status_rsp(struct bt_mesh_dfd_srv *srv,
                          struct bt_mesh_msg_ctx *ctx,
                          enum bt_mesh_dfd_status status, u16_t idx,
                          const u8_t *fwid, size_t fwid_len)
{
                      ...
                      enum bt_mesh_dfu_status reason)
{
    ...
    LOG_DBG(">>> %s reason: %u, phase: %u, apply: %u", __func__, reason, srv->phase, srv->apply);
...
static void upload_end(struct bt_mesh_blob_srv *b, u64_t id, bool success)
{
    struct bt_mesh_dfd_srv *srv =
...
enum bt_mesh_dfd_status bt_mesh_dfd_srv_receiver_add(struct bt_mesh_dfd_srv *srv, u16_t addr,
        u8_t img_idx)
{
        ...
        struct bt_mesh_dfd_start_params *params)
{
    ...
    for (i = 0; i < srv->target_cnt; i++) {
        u16_t addr = srv->targets[i].blob.addr;
    ...
    LOG_DBG("Distribution Start: slot: %d, appidx: %d, tb: %d, group addr: %04X, ttl: %d, apply: %d",
...
int bt_mesh_dfd_srv_oob_store_complete(struct bt_mesh_dfd_srv *srv,
                                       ...
                                       size_t size, const u8_t *metadata, size_t metadata_len)
{
    int err = 0;
