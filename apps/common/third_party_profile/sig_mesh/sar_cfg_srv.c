...
#pragma bss_seg(".ble_mesh_sarcfgsrv_bss")
#pragma data_seg(".ble_mesh_sarcfgsrv_data")
#pragma const_seg(".ble_mesh_sarcfgsrv_const")
#pragma code_seg(".ble_mesh_sarcfgsrv_code")
#endif
                                    ...
                                    void *cb_data)
{
    ...
    err = node_info_load(SAR_RX_STATE_INDEX, &bt_mesh.sar_rx,
                         sizeof(bt_mesh.sar_rx));
    if (err) {
        LOG_ERR("sar_rx_cfg_srv_settings_set load info failed!!");
        return err;
    }

    err = node_info_load(SAR_TX_STATE_INDEX, &bt_mesh.sar_tx,
                         sizeof(bt_mesh.sar_tx));
    if (err) {
        LOG_ERR("sar_tx_cfg_srv_settings_set load info failed!!");
        return err;
    }
