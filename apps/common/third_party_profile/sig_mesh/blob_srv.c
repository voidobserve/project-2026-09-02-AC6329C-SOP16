...
#pragma bss_seg(".ble_mesh_blobsrv_bss")
#pragma data_seg(".ble_mesh_blobsrv_data")
#pragma const_seg(".ble_mesh_blobsrv_const")
#pragma code_seg(".ble_mesh_blobsrv_code")
#endif
...
#define SERVER_TIMEOUT_SECS(srv) (10 * (5 + (srv)->state.timeout_base))
...
static inline u32_t block_count_get(const struct bt_mesh_blob_srv *srv)
{
    return DIV_ROUND_UP(srv->state.xfer.size,
                        (1U << srv->state.xfer.block_size_log));
}

static inline u32_t max_chunk_size(const struct bt_mesh_blob_srv *srv)
{
...
static inline u32_t max_chunk_count(const struct bt_mesh_blob_srv *srv)
{
    return MIN(8 * (srv->state.mtu_size - 6),
               CONFIG_BT_MESH_BLOB_CHUNK_COUNT_MAX);
}

static inline u32_t missing_chunks(const struct bt_mesh_blob_block *block)
{
    int i;
    u32_t count = 0;
...
static void store_state(const struct bt_mesh_blob_srv *srv)
{
    ...
    u32_t block_len = DIV_ROUND_UP(block_count_get(srv), 8);

    bt_mesh_model_data_store(
        srv->mod, false, "blob", &srv->state,
        offsetof(struct bt_mesh_blob_srv_state, blocks) + block_len);
    LOG_INF("blob store state size %d", offsetof(struct bt_mesh_blob_srv_state, blocks) + block_len);
}
...
static void erase_state(struct bt_mesh_blob_srv *srv)
{
    ...
    bt_mesh_model_data_store(srv->mod, false, "blob", &srv->state, 0);
    LOG_INF("blob erase state size %d", sizeof(srv->state));
}
...
static void buf_chunk_index_add(struct net_buf_simple *buf, u16_t chunk)
{
    /* utf-8 encoded: */
    if (chunk < 0x80) {
...
static int pull_req_max(const struct bt_mesh_blob_srv *srv)
{
    ...
    if (bt_mesh_lpn_established()) {
        u32_t segments_per_chunk = DIV_ROUND_UP(
...
static bool all_blocks_received(struct bt_mesh_blob_srv *srv)
{
    for (int i = 0; i < ARRAY_SIZE(srv->state.blocks); ++i) {
        if (srv->state.blocks[i]) {
            LOG_ERR("block %d is not received.", i);
            return false;
        }
    }
                             ...
                             enum bt_mesh_blob_status status)
{
    ...
    LOG_INF(">>> %s: Status: %u, missing: %u/%u", __func__, status, missing, srv->block.chunk_count);
                             ...
                             struct net_buf_simple *buf)
{
    ...
    if (srv->state.xfer.id != id) {
         ...
         */
        LOG_WRN("Invalid ID: %s", bt_hex(&id, sizeof(u64_t)));
        LOG_WRN("Expected ID: %s", bt_hex(&srv->state.xfer.id, sizeof(u64_t)));
        goto rsp;
    }
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (srv->phase == BT_MESH_BLOB_XFER_PHASE_WAITING_FOR_CHUNK) {
        ...
        LOG_WRN("xfer phase waiting for chunk.");
        goto rsp;
    }

    if (block_number >= block_count_get(srv)) {
        status = BT_MESH_BLOB_ERR_INVALID_BLOCK_NUM;
        LOG_WRN("invalid blonk num %s.", block_number);
        goto rsp;
    }
    ...
    if (srv->io->block_start) {
        ...
        if (err) {
            cancel(srv);
            status = BT_MESH_BLOB_ERR_INTERNAL;
            LOG_WRN("%s error inernal.", __func__);
            goto rsp;
        }
    }
                        ...
                        struct net_buf_simple *buf)
{
    ...
    if (srv->phase != BT_MESH_BLOB_XFER_PHASE_WAITING_FOR_CHUNK ||
        ...
        LOG_ERR("Invalid phase or index (%u %u %u)", srv->phase,
                idx, srv->block.chunk_count);
        return -EINVAL;
    }
    ...
    LOG_INF(">>> %s: %u/%u (%u bytes)", __func__, idx + 1, srv->block.chunk_count,
            chunk.size);
                                 ...
                                 void *cb_arg)
{
    ...
    err = node_info_load(BLOB_SRV_STATE_INDEX, &srv->state, sizeof(srv->state));
    if (err) {
        LOG_ERR("blob_srv_settings_set load info failed size %d !!", sizeof(srv->state));
        return err;
    }
...
int bt_mesh_blob_srv_recv(struct bt_mesh_blob_srv *srv, u64_t id,
                          const struct bt_mesh_blob_io *io, u8_t ttl,
                          u16_t timeout_base)
{
    if (bt_mesh_blob_srv_is_busy(srv)) {
        LOG_ERR(">>> %s blob srv busy.", __func__);
        return -EBUSY;
    }

    if (!io || !io->wr) {
        LOG_ERR(">>> %s io & io write invalid.", __func__);
        return -EINVAL;
    }
...
u8_t bt_mesh_blob_srv_progress(const struct bt_mesh_blob_srv *srv)
{
    u32_t total;
    u32_t received;
