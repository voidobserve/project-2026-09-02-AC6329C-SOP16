...
#pragma bss_seg(".ble_mesh_rprsrv_bss")
#pragma data_seg(".ble_mesh_rprsrv_data")
#pragma const_seg(".ble_mesh_rprsrv_const")
#pragma code_seg(".ble_mesh_rprsrv_code")
#endif
...
static struct {
    ...
    struct {
        ...
        /* Extended scanning */
        bt_addr_le_t addr;
        u8_t ad[CONFIG_BT_MESH_RPR_AD_TYPES_MAX];
        u8_t ad_count;
        /* Time to do regular scanning after extended scanning ends: */
        u32_t additional_time;
...
static struct bt_mesh_rpr_unprov *unprov_get(const u8_t uuid[16])
{
    int i;
...
static u8_t *get_ad_type(u8_t *list, size_t count, u8_t ad)
{
    int i;
                             ...
                             enum bt_mesh_rpr_status status)
{
    ...
    if (atomic_test_bit(srv.flags, SCANNING)) {
        // timeout = k_ticks_to_ms_floor32(	//for compiler, not used now.
...
static void scan_report_schedule(void)
{
    ...
    (void)bt_rand(&delay, sizeof(u32_t));
    delay = (delay % 480) + 20;
...
static void scan_ext_stop(u32_t remaining_time)
{
                                      ...
                                      struct net_buf_simple *buf)
{
    ...
    if (srv.scan.state == BT_MESH_RPR_SCAN_IDLE) {
        ...
        srv.scan.cli = cli;
        // } else if (k_ticks_to_ms_floor32(	//for compiler, not used now.
        // 		k_work_delayable_remaining_get(&srv.scan.timeout)) <
                            ...
                            struct net_buf_simple *buf)
{
    ...
    struct bt_mesh_rpr_node cli = RPR_NODE(ctx);
    s8_t timeout = LINK_OPEN_TIMEOUT_DEFAULT;
    enum bt_mesh_rpr_status status;
    const u8_t *uuid;
    u8_t refresh;
    int err;
