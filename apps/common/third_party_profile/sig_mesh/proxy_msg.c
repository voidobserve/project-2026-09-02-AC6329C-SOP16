...
#pragma bss_seg(".ble_mesh_proxymsg_bss")
#pragma data_seg(".ble_mesh_proxymsg_data")
#pragma const_seg(".ble_mesh_proxymsg_const")
#pragma code_seg(".ble_mesh_proxymsg_code")
#endif
...
static u8_t __noinit bufs[CONFIG_BT_MAX_CONN * CONFIG_BT_MESH_PROXY_MSG_LEN];
...
int bt_mesh_proxy_msg_send(struct bt_conn *conn, u8_t type,
...
int bt_mesh_proxy_relay_send(struct bt_conn *conn, struct bt_mesh_adv *adv)
{
    ...
    bt_mesh_adv_send_end(err, &adv->ctx);	//Fix: add this to advoid end_cb not run in proxy connect.
    return err;
}
