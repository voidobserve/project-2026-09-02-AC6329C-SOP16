...
#pragma bss_seg(".ble_mesh_provisioner_bss")
#pragma data_seg(".ble_mesh_provisioner_data")
#pragma const_seg(".ble_mesh_provisioner_const")
#pragma code_seg(".ble_mesh_provisioner_code")
#endif
    ...
    if (bt_mesh_prov_link.oob_method == AUTH_METHOD_STATIC) {
        ...
        if (!(BIT(bt_mesh_prov_link.oob_action) & caps->input_actions)) {
            LOG_WRN("The required input action (0x%04x) "
                    ...
                    (u16_t)BIT(bt_mesh_prov_link.oob_action), caps->input_actions);
            return false;
        }
        ...
        if (!(BIT(bt_mesh_prov_link.oob_action) & caps->output_actions)) {
            LOG_WRN("The required output action (0x%04x) "
                    "not supported by the device (0x%02x)",
                    (u16_t)BIT(bt_mesh_prov_link.oob_action), caps->output_actions);
            return false;
        }
...
static void send_confirm(void)
{
    ...
    u8_t auth_size = bt_mesh_prov_auth_size_get();
    u8_t *inputs = (u8_t *)&bt_mesh_prov_link.conf_inputs;
    u8_t conf_key_input[64];
...
static void prov_dh_key_gen(void)
{
    const u8_t *remote_pk;
    const u8_t *local_pk;
    u8_t remote_pk_le[64];
...
static void provis_complete(const u8_t *data)
{
...
static void send_random(void)
{
    ...
    u8_t rand_size = bt_mesh_prov_auth_size_get();
...
{
    u8_t conf_size = bt_mesh_prov_auth_size_get();
...
static const struct bt_mesh_prov_role role_provisioner = {
    ...
    .op = {
        ...
        [PROV_COMPLETE] = provis_complete,
        [PROV_FAILED] = prov_failed,
    },
};

static void prov_set_method(u8_t method, u8_t action, u8_t size)
{
...
int bt_mesh_auth_method_set_input(bt_mesh_input_action_t action, u8_t size)
{
...
int bt_mesh_auth_method_set_output(bt_mesh_output_action_t action, u8_t size)
{
    if (!action || !size || size > PROV_IO_OOB_SIZE_MAX) {
        return -EINVAL;
    }
...
int bt_mesh_auth_method_set_static(const u8_t *static_val, u8_t size)
{
    if (!size || !static_val) {
        return -EINVAL;
    }
...
int bt_mesh_prov_remote_pub_key_set(const u8_t public_key[PUB_KEY_SIZE])
{
    if (public_key == NULL) {
        return -EINVAL;
    }
...
static int link_open(const u8_t *uuid, const struct prov_bearer *bearer,
                     u16_t net_idx, u16_t addr,
                     u8_t attention_duration, void *bearer_cb_data, u8_t timeout)
{
    int err;
...
int bt_mesh_pb_gatt_open(const u8_t uuid[16], u16_t net_idx, u16_t addr,
                         u8_t attention_duration)
{
...
int bt_mesh_pb_remote_open(struct bt_mesh_rpr_cli *cli,
                           const struct bt_mesh_rpr_node *srv, const u8_t uuid[16],
                           u16_t net_idx, u16_t addr)
{
 ...
 */
static int reprovision_local_client_server(u16_t addr)
{
    int err;
    const u8_t *pub_key;
    const u8_t *priv_key = NULL;
...
int bt_mesh_pb_remote_open_node(struct bt_mesh_rpr_cli *cli,
                                struct bt_mesh_rpr_node *srv, u16_t addr,
