...
struct bt_mesh_beacon {
    u32_t sent;        /* Timestamp of last sent beacon */
    u32_t recv;        /* Timestamp of last received beacon */
    u8_t  last;       /* Number of beacons during last
				      * observation window
				      */
    u8_t  cur;        /* Number of beacons observed during
				      * currently ongoing window.
				      */
    u8_t  cache[8];   /* Cached last beacon auth value */
...
struct bt_mesh_subnet {
    ...
    u32_t priv_net_id_sent; /* Timestamp for Private Network ID advertising
...
int bt_mesh_friend_cred_create(struct bt_mesh_net_cred *cred,
                               u16_t lpn_addr, u16_t frnd_addr,
