...
#define OFFSET(item) (item & (u8_t)BIT_MASK(5))
...
static u8_t state_status_u8(const struct bt_mesh_model *model,
                            struct bt_mesh_msg_ctx *ctx,
                            struct net_buf_simple *buf,
                            u32_t expect_status)
{
...
int bt_mesh_key_idx_unpack_list(struct net_buf_simple *buf, u16_t *dst_arr,
                                size_t *dst_cnt)
{
    size_t i;
...
static int mod_sub_list_handle(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf, u16_t op,
                               bool vnd)
{
    int err = 0;
...
static int mod_app_list_handle(struct bt_mesh_msg_ctx *ctx, struct net_buf_simple *buf, u16_t op,
                               bool vnd)
{
    int err = 0;
    struct net_buf_simple_state state;
...
static int get_state_u8(u16_t net_idx, u16_t addr, u32_t op, u32_t rsp, u8_t *val)
{
...
static int set_state_u8(u16_t net_idx, u16_t addr, u32_t op, u32_t rsp, u8_t new_val,
                        u8_t *val)
{
...
static int mod_app_bind(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_app_idx,
...
static int mod_app_unbind(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_app_idx,
                          u16_t mod_id, u16_t cid, u8_t *status)
{
...
static int mod_member_list_get(u32_t op, u32_t expect_op, u16_t net_idx, u16_t addr,
                               u16_t elem_addr, u16_t mod_id, u16_t cid, u8_t *status,
...
static int mod_pub_get(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_id,
                       u16_t cid, struct bt_mesh_cfg_cli_mod_pub *pub, u8_t *status)
{
...
static int mod_pub_set(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_id,
                       u16_t cid, struct bt_mesh_cfg_cli_mod_pub *pub, u8_t *status)
{
...
static int mod_pub_va_set(u16_t net_idx, u16_t addr, u16_t elem_addr, u16_t mod_id,
                          u16_t cid, struct bt_mesh_cfg_cli_mod_pub *pub, u8_t *status)
{
...
struct bt_mesh_mod_id_vnd bt_mesh_comp_p0_elem_mod_vnd(struct bt_mesh_comp_p0_elem *elem, int idx)
{
    CHECKIF(idx >= elem->nvnd) {
        return (struct bt_mesh_mod_id_vnd) {
            0xffff, 0xffff
        };
    }
    ...
    struct bt_mesh_comp_p1_model_item *item, struct bt_mesh_comp_p1_item_short *ext_item)
{
    ...
    u8_t item_data = net_buf_simple_pull_u8(item->_buf);
