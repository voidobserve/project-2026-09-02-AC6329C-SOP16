...
struct bt_mesh_msg_ack_ctx {
    OS_SEM          sem;       /**< Sync semaphore. */ //for compiler, not used now.
...
int bt_mesh_msg_ack_ctx_prepare(struct bt_mesh_msg_ack_ctx *ack,
                                uint32_t op, u16_t dst, void *user_data);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_msg_ack_ctx_wait(struct bt_mesh_msg_ack_ctx *ack, u32_t timeout);
...
bool bt_mesh_msg_ack_ctx_match(const struct bt_mesh_msg_ack_ctx *ack,
                               uint32_t op, u16_t addr, void **user_data);
