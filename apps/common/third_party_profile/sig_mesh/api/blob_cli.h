...
struct bt_mesh_blob_target_pull {
    ...
    s64_t block_report_timestamp;

    /** Missing chunks reported by this Target node. */
    u8_t missing[DIV_ROUND_UP(CONFIG_BT_MESH_BLOB_CHUNK_COUNT_MAX, 8)];
};
...
struct bt_mesh_blob_target {
    ...
    u8_t procedure_complete: 1, /* Procedure has been completed. */
...
struct blob_cli_broadcast_ctx {
    ...
    void (*send)(struct bt_mesh_blob_cli *cli, u16_t dst);
    /** Called after every @ref blob_cli_broadcast_ctx::send callback. */
    void (*send_complete)(struct bt_mesh_blob_cli *cli, u16_t dst);
