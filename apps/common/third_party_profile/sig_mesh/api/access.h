...
#define BT_MESH_MODEL_UUIDS_UNASSIGNED()			\
	.uuids = (const u8_t *[]){ LISTIFY(CONFIG_BT_MESH_LABEL_COUNT, \
...
{										\
	...
	.pub = _pub,								\
	.keys = (u16_t []) BT_MESH_MODEL_KEYS_UNUSED(_keys),			\
	.keys_cnt = _keys,							\
	.groups = (u16_t []) BT_MESH_MODEL_GROUPS_UNASSIGNED(_grps),		\
	.groups_cnt = _grps,							\
...
{                                                                            \
	...
	BT_MESH_MODEL_RUNTIME_INIT(_user_data)				     \
	.pub = _pub,                                                         \
	.keys = (u16_t []) BT_MESH_MODEL_KEYS_UNUSED(CONFIG_BT_MESH_MODEL_KEY_COUNT), \
	.keys_cnt = CONFIG_BT_MESH_MODEL_KEY_COUNT,                          \
	.groups = (u16_t []) BT_MESH_MODEL_GROUPS_UNASSIGNED(CONFIG_BT_MESH_MODEL_GROUP_COUNT), \
...
struct bt_mesh_model_pub {
    ...
    const u8_t *uuid;    /**< Label UUID if Publish Address is Virtual Address. */
    ...
    u8_t delayable: 1;  /**< Use random delay for publishing. */
