...
struct bt_mesh_prov {
     ...
     *  @param oob_info OOB Information
     */
    void (*unprovisioned_beacon_gatt)(u8_t uuid[16],
                                      bt_mesh_prov_oob_info_t oob_info);
 ...
 *  @return Zero on success or (negative) error code otherwise.
 */
int bt_mesh_prov_remote_pub_key_set(const u8_t public_key[64]);
 ...
 *  @return Zero on success or (negative) error code otherwise.
 */
int bt_mesh_provision_gatt(const u8_t uuid[16], u16_t net_idx, u16_t addr,
                           u8_t attention_duration);
...
int bt_mesh_provision_remote(struct bt_mesh_rpr_cli *cli,
                             ...
                             const u8_t uuid[16], u16_t net_idx,
                             u16_t addr);
...
struct bt_mesh_lpn_cb {
     ...
     *  Friend response.
     */
    void (*established)(u16_t net_idx, u16_t friend_addr,
                        u8_t queue_size, u8_t recv_window);
     ...
     *  @param friend_addr Friend address.
     */
    void (*terminated)(u16_t net_idx, u16_t friend_addr);
     ...
     */
    void (*polled)(u16_t net_idx, u16_t friend_addr, bool retry);
};
...
struct bt_mesh_friend_cb {
     ...
     */
    void (*established)(u16_t net_idx, u16_t lpn_addr,
                        u8_t recv_delay, u32_t polltimeout);
     ...
     *  @param lpn_addr Low Power Node address.
     */
    void (*terminated)(u16_t net_idx, u16_t lpn_addr);
     ...
     */
    void (*polled)(u16_t net_idx, u16_t lpn_addr);
};
