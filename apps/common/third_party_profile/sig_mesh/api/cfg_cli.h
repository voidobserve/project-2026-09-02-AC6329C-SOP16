...
struct bt_mesh_cfg_cli_cb {
     ...
     */
    void (*comp_data)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t page,
     ...
     */
    void (*mod_pub_status)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
     ...
     */
    void (*mod_sub_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                           u8_t status, u16_t elem_addr,
                           u16_t sub_addr, u32_t mod_id);
     ...
     */
    void (*mod_sub_list)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
                         u16_t elem_addr, u16_t mod_id, u16_t cid,
                         struct net_buf_simple *buf);
     ...
     *  @param addr        Address of the sender.
     */
    void (*node_reset_status)(struct bt_mesh_cfg_cli *cli, u16_t addr);
     ...
     *  @param status      Status Code for requesting message.
     */
    void (*beacon_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                          u8_t status);
     ...
     *  @param status      Status Code for requesting message.
     */
    void (*ttl_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                       u8_t status);
     ...
     *  @param status      Status Code for requesting message.
     */
    void (*friend_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                          u8_t status);
     ...
     *  @param status      Status Code for requesting message.
     */
    void (*gatt_proxy_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                              u8_t status);
     ...
     *  @param status      Status Code for requesting message.
     */
    void (*network_transmit_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                                    u8_t status);
     ...
     */
    void (*relay_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                         u8_t status, u8_t transmit);
     ...
     */
    void (*net_key_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                           u8_t status, u16_t net_idx);
     ...
     */
    void (*net_key_list)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                         struct net_buf_simple *buf);
     ...
     */
    void (*app_key_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                           u8_t status, u16_t net_idx,
                           u16_t app_idx);
     ...
     *  @param buf     Message buffer containing key indexes.
     */
    void (*app_key_list)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
                         u16_t net_idx, struct net_buf_simple *buf);
     ...
     *  @param mod_id      The model ID within the element.
     */
    void (*mod_app_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                           u8_t status, u16_t elem_addr,
                           u16_t app_idx, u32_t mod_id);
     ...
     *  @param buf       Message buffer containing key indexes.
     */
    void (*mod_app_list)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
                         u16_t elem_addr, u16_t mod_id, u16_t cid,
                         struct net_buf_simple *buf);
     ...
     */
    void (*node_identity_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                                 u8_t status, u16_t net_idx,
                                 u8_t identity);
     ...
     */
    void (*lpn_timeout_status)(struct bt_mesh_cfg_cli *cli, u16_t addr,
                               u16_t elem_addr, u32_t timeout);
     ...
     */
    void (*krp_status)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
                       u16_t net_idx, u8_t phase);
     ...
     */
    void (*hb_pub_status)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
     ...
     */
    void (*hb_sub_status)(struct bt_mesh_cfg_cli *cli, u16_t addr, u8_t status,
 ...
 */
int bt_mesh_cfg_cli_node_reset(u16_t net_idx, u16_t addr, bool *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_comp_data_get(u16_t net_idx, u16_t addr, u8_t page, u8_t *rsp,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_beacon_get(u16_t net_idx, u16_t addr, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_krp_get(u16_t net_idx, u16_t addr, u16_t key_net_idx, u8_t *status,
                            u8_t *phase);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_krp_set(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_beacon_set(u16_t net_idx, u16_t addr, u8_t val, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_ttl_get(u16_t net_idx, u16_t addr, u8_t *ttl);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_ttl_set(u16_t net_idx, u16_t addr, u8_t val, u8_t *ttl);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_friend_get(u16_t net_idx, u16_t addr, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_friend_set(u16_t net_idx, u16_t addr, u8_t val, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_gatt_proxy_get(u16_t net_idx, u16_t addr, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_gatt_proxy_set(u16_t net_idx, u16_t addr, u8_t val, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_transmit_get(u16_t net_idx, u16_t addr, u8_t *transmit);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_transmit_set(u16_t net_idx, u16_t addr, u8_t val,
                                     u8_t *transmit);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_relay_get(u16_t net_idx, u16_t addr, u8_t *status, u8_t *transmit);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_relay_set(u16_t net_idx, u16_t addr, u8_t new_relay,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_key_add(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_key_get(u16_t net_idx, u16_t addr, u16_t *keys, size_t *key_cnt);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_key_del(u16_t net_idx, u16_t addr, u16_t key_net_idx,
                                u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_app_key_add(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_app_key_get(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_app_key_del(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_bind(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_unbind(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                   u16_t mod_app_idx, u16_t mod_id, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_bind_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_unbind_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                       u16_t mod_app_idx, u16_t mod_id, u16_t cid,
                                       u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_get(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                u16_t mod_id, u8_t *status, u16_t *apps, size_t *app_cnt);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_app_get_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                    u16_t mod_id, u16_t cid, u8_t *status, u16_t *apps,
                                    size_t *app_cnt);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_pub_get(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                u16_t mod_id, struct bt_mesh_cfg_cli_mod_pub *pub,
                                u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_pub_get_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                    u16_t mod_id, u16_t cid,
                                    struct bt_mesh_cfg_cli_mod_pub *pub, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_pub_set(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                u16_t mod_id, struct bt_mesh_cfg_cli_mod_pub *pub,
                                u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_pub_set_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                    u16_t mod_id, u16_t cid,
                                    struct bt_mesh_cfg_cli_mod_pub *pub, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_add(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_add_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_del(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                u16_t sub_addr, u16_t mod_id, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_del_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                    u16_t sub_addr, u16_t mod_id, u16_t cid,
                                    u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_overwrite(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                      u16_t sub_addr, u16_t mod_id, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_overwrite_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
        u16_t sub_addr, u16_t mod_id, u16_t cid,
        u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_add(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_add_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_del(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                   const u8_t label[16], u16_t mod_id, u16_t *virt_addr,
                                   u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_del_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                       const u8_t label[16], u16_t mod_id, u16_t cid,
                                       u16_t *virt_addr, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_overwrite(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_va_overwrite_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
        const u8_t label[16], u16_t mod_id, u16_t cid,
        u16_t *virt_addr, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_get(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                u16_t mod_id, u8_t *status, u16_t *subs, size_t *sub_cnt);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_get_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
                                    u16_t mod_id, u16_t cid, u8_t *status, u16_t *subs,
                                    size_t *sub_cnt);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_hb_sub_set(u16_t net_idx, u16_t addr, struct bt_mesh_cfg_cli_hb_sub *sub,
                               u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_hb_sub_get(u16_t net_idx, u16_t addr, struct bt_mesh_cfg_cli_hb_sub *sub,
                               u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_hb_pub_set(u16_t net_idx, u16_t addr,
                               const struct bt_mesh_cfg_cli_hb_pub *pub, u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_hb_pub_get(u16_t net_idx, u16_t addr, struct bt_mesh_cfg_cli_hb_pub *pub,
                               u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_del_all(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_mod_sub_del_all_vnd(u16_t net_idx, u16_t addr, u16_t elem_addr,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_net_key_update(u16_t net_idx, u16_t addr, u16_t key_net_idx,
                                   const u8_t net_key[16], u8_t *status);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_app_key_update(u16_t net_idx, u16_t addr, u16_t key_net_idx,
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_node_identity_set(u16_t net_idx, u16_t addr, u16_t key_net_idx,
                                      u8_t new_identity, u8_t *status, u8_t *identity);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_node_identity_get(u16_t net_idx, u16_t addr, u16_t key_net_idx,
                                      u8_t *status, u8_t *identity);
 ...
 *  @return 0 on success, or (negative) error code on failure.
 */
int bt_mesh_cfg_cli_lpn_timeout_get(u16_t net_idx, u16_t addr, u16_t unicast_addr,
                                    s32_t *polltimeout);
 ...
 */
u16_t bt_mesh_comp_p0_elem_mod(struct bt_mesh_comp_p0_elem *elem, int idx);
 ...
 */
int bt_mesh_key_idx_unpack_list(struct net_buf_simple *buf, u16_t *dst_arr, size_t *dst_cnt);
