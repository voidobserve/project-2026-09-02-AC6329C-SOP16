...
#pragma bss_seg(".ble_mesh_blobcli_bss")
#pragma data_seg(".ble_mesh_blobcli_data")
#pragma const_seg(".ble_mesh_blobcli_const")
#pragma code_seg(".ble_mesh_blobcli_code")
#endif
...
static void start_retry_timer(struct bt_mesh_blob_cli *cli)
{
    ...
    if (SENDING_CHUNKS_IN_PULL_MODE(cli)) {
        s64_t next_timeout_ms = cli->tx.cli_timestamp;
...
static u32_t targets_reset(struct bt_mesh_blob_cli *cli)
{
    struct bt_mesh_blob_target *target;
    u32_t count = 0;
...
static u16_t next_missing_chunk(struct bt_mesh_blob_cli *cli,
                                const u8_t *missing_chunks,
                                u16_t idx)
{
    do {
...
static void block_set(struct bt_mesh_blob_cli *cli, u16_t block_idx)
{
    ...
    LOG_INF(">>> %s: %u size: %u chunks: %u", __func__, block_idx, cli->block.size,
            cli->block.chunk_count);
}
...
{
    LOG_INF(">>> %s: %s", __func__, cli->tx.cancelled ? "cancelling" : "continuing");
...
static void retry_timeout(struct k_work *work)
{
    ...
    LOG_INF(">>> %s %u", __func__, cli->tx.retries);
                        ...
                        const struct blob_cli_broadcast_ctx *ctx)
{
    ...
    LOG_INF(">>> %s %u", __func__, cli->tx.pending);
                            ...
                            struct bt_mesh_blob_target *target)
{
    ...
    LOG_INF(">>> %s: 0x%04x, pending: %d sending: %d ", __func__, target->addr, cli->tx.pending, cli->tx.sending);
...
static void send_start(u16_t duration, int err, void *cb_data);
static void send_end(int err, void *user_data);

static int tx(struct bt_mesh_blob_cli *cli, u16_t addr,
...
static void info_get_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void xfer_start_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void xfer_get_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void xfer_cancel_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void block_start_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void chunk_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void block_get_tx(struct bt_mesh_blob_cli *cli, u16_t dst)
{
...
static void block_start(struct bt_mesh_blob_cli *cli)
{
    ...
    LOG_INF(">>> %s: %u (%u chunks, %u/%u)", __func__, cli->block.number,
...
static void chunk_tx_complete(struct bt_mesh_blob_cli *cli, u16_t dst)
{
    if (cli->xfer->mode != BT_MESH_BLOB_XFER_MODE_PULL) {
        return;
    }
     ...
     */
    u16_t chunk_idx = next_missing_chunk(cli, cli->tx.target->pull->missing,
                                         cli->chunk_idx + 1);
     ...
     */
    struct bt_mesh_blob_target *target;
    s64_t timestamp = k_uptime_get() + BLOCK_REPORT_TIME_MSEC;
...
static void chunk_send(struct bt_mesh_blob_cli *cli)
{
    ...
    LOG_INF(">>> %s: %u / %u size: %u", __func__, cli->chunk_idx + 1, cli->block.chunk_count,
...
static void block_check_end(struct bt_mesh_blob_cli *cli)
{
    ...
    LOG_INF("No more missing chunks for block %u", cli->block.number);
                            ...
                            struct block_status *block)
{
    ...
    LOG_INF(">>> %s: 0x%04x: block: %u status: %u", __func__, target->addr,
            block->block.number, block->status);
                              ...
                              struct net_buf_simple *buf)
{
    ...
    LOG_DBG("status: %u %s phase: %u id: %s", info.status,
    ...
    if (!target) {
        LOG_ERR("%s L %d no target.", __func__, __LINE__);
        return -ENOENT;
    }
                               ...
                               struct net_buf_simple *buf)
{
    ...
    if (!cli->xfer) {
        LOG_ERR("%s L %d no xfer.", __func__, __LINE__);
        return -EINVAL;
    }
                               ...
                               struct net_buf_simple *buf)
{
    ...
    LOG_INF(">>> %s: status: %u block: %u encoding: %u", __func__, status.status,
    ...
    case BT_MESH_BLOB_CHUNKS_MISSING_SOME:
        if (buf->len > sizeof(status.block.missing)) {
            LOG_ERR("%s L %d invalid missing block.", __func__, __LINE__);
            return -EINVAL;
        }
                              ...
                              struct net_buf_simple *buf)
{
    ...
    if (cli->state != BT_MESH_BLOB_CLI_STATE_CAPS_GET) {
        LOG_ERR("%s L %d cli->state %d", __func__, __LINE__, cli->state);
        return -EBUSY;
    }
    ...
    if (caps.min_block_size_log < 0x06 ||
        ...
        LOG_ERR("%s L %d caps para failed.", __func__, __LINE__);
        return -EINVAL;
    }
...
u8_t bt_mesh_blob_cli_xfer_progress_active_get(struct bt_mesh_blob_cli *cli)
{
    if (cli->state < BT_MESH_BLOB_CLI_STATE_START) {
        return 0;
    }
