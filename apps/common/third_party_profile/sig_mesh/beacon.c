...
static void cache_add(const u8_t auth[8], struct bt_mesh_beacon *beacon)
{
...
static void beacon_start(u16_t duration, int err, void *user_data)
{
    if (err) {
...
{
    u8_t interval = bt_mesh_priv_beacon_update_interval_get();
                                   ...
                                   struct net_buf_simple *buf))
{
    ...
    if (!err) {
        LOG_ERR("%s L %d ", __func__, __LINE__);
