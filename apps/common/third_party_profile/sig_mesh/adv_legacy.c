...
#pragma bss_seg(".ble_mesh_advlegacy_bss")
#pragma data_seg(".ble_mesh_advlegacy_data")
#pragma const_seg(".ble_mesh_advlegacy_const")
#pragma code_seg(".ble_mesh_advlegacy_code")
#endif
...
// static K_KERNEL_STACK_DEFINE(adv_thread_stack, CONFIG_BT_MESH_ADV_STACK_SIZE);//for compiler, not used now.
static s32_t adv_timeout;
static bool enabled;

static int bt_data_send(u8_t num_events, u16_t adv_int,
                        const struct bt_data *ad, size_t ad_len,
                        struct bt_mesh_adv_ctx *ctx)
{
    struct bt_le_adv_param param = {};
    u64_t uptime = k_uptime_get();
    u16_t duration;
    int err;
    const s32_t adv_int_min = 10;
    // ((bt_dev.hci_version >= BT_HCI_VERSION_5_0) ?	//for compiler, not used now.
    //  ADV_INT_FAST_MS :
    //  ADV_INT_DEFAULT_MS);
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_DEBUG_USE_ID_ADDR)) {
        param.options = 1U;//BT_LE_ADV_OPT_USE_IDENTITY;	//for compiler, not used now.
    } else {
        param.options = 0U;
    }
    ...
    LOG_DBG("Advertising stopped (%u ms)", (u32_t) k_uptime_delta(&uptime));

    return 0;
}

// int bt_mesh_adv_bt_data_send(u8_t num_events, u16_t adv_int,
// 			     const struct bt_data *ad, size_t ad_len)
// {
...
{
    u16_t num_events = BT_MESH_TRANSMIT_COUNT(adv->ctx.xmit) + 1;
    u16_t adv_int;
    struct bt_data ad;
...
// {
// k_thread_create(&adv_thread_data, adv_thread_stack,	//for compiler, not used now.
