...
#pragma bss_seg(".ble_mesh_subnet_bss")
#pragma data_seg(".ble_mesh_subnet_data")
#pragma const_seg(".ble_mesh_subnet_const")
#pragma code_seg(".ble_mesh_subnet_code")
#endif
...
{
    // STRUCT_SECTION_FOREACH(bt_mesh_subnet_cb, cb) {	//for compiler, not used now.
    // 	cb->evt_handler(sub, evt);
    // }
}


static struct net_key_update *net_key_update_find(u16_t key_idx,
...
static void update_subnet_settings(u16_t net_idx, bool store)
{
...
void bt_mesh_subnet_store(u16_t net_idx)
{
...
static void key_refresh(struct bt_mesh_subnet *sub, u8_t new_phase)
{
...
static int msg_cred_create(struct bt_mesh_net_cred *cred, const u8_t *p,
                           size_t p_len, const u8_t key[16])
{
    return bt_mesh_k2(key, p, p_len, &cred->nid, &cred->enc, &cred->privacy);
}

static int net_keys_create(struct bt_mesh_subnet_keys *keys, bool import, const u8_t key[16])
{
    u8_t p = 0;
    int err;
...
u8_t bt_mesh_subnet_add(u16_t net_idx, const u8_t key[16])
{
    struct bt_mesh_subnet *sub = NULL;
    int err;
...
bool bt_mesh_subnet_exists(u16_t net_idx)
{
    return !!bt_mesh_subnet_get(net_idx);
}

u8_t bt_mesh_subnet_update(u16_t net_idx, const u8_t key[16])
{
...
u8_t bt_mesh_subnet_del(u16_t net_idx)
{
    struct bt_mesh_subnet *sub;
...
int bt_mesh_friend_cred_create(struct bt_mesh_net_cred *cred, u16_t lpn_addr,
                               u16_t frnd_addr, u16_t lpn_counter,
                               u16_t frnd_counter, const struct bt_mesh_key *key)
{
    u8_t p[9];
    u8_t raw_key[16];
    int err;
...
u8_t bt_mesh_subnet_kr_phase_set(u16_t net_idx, u8_t *phase)
{
    /* Table in MshPRTv1.1: 4.2.15: */
    const u8_t valid_transitions[] = {
...
u8_t bt_mesh_subnet_kr_phase_get(u16_t net_idx, u8_t *phase)
{
    struct bt_mesh_subnet *sub;
...
u8_t bt_mesh_subnet_node_id_set(u16_t net_idx,
...
u8_t bt_mesh_subnet_node_id_get(u16_t net_idx,
                                enum bt_mesh_feat_state *node_id)
{
    struct bt_mesh_subnet *sub;
...
u8_t bt_mesh_subnet_priv_node_id_set(u16_t net_idx,
...
u8_t bt_mesh_subnet_priv_node_id_get(u16_t net_idx,
                                     enum bt_mesh_feat_state *priv_node_id)
{
    struct bt_mesh_subnet *sub;
...
ssize_t bt_mesh_subnets_get(u16_t net_idxs[], size_t max, off_t skip)
{
    size_t count = 0;
...
int bt_mesh_subnet_set(u16_t net_idx, u8_t kr_phase, const struct bt_mesh_key *old_key,
