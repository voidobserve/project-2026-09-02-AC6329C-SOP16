...
#pragma bss_seg(".ble_mesh_dfusrv_bss")
#pragma data_seg(".ble_mesh_dfusrv_data")
#pragma const_seg(".ble_mesh_dfusrv_const")
#pragma code_seg(".ble_mesh_dfusrv_code")
#endif
...
{
    bt_mesh_model_data_store(srv->mod, false, "dfu", &srv->update,
                             sizeof(srv->update));
}

static void erase_state(struct bt_mesh_dfu_srv *srv)
{
    bt_mesh_model_data_store(srv->mod, false, "dfu", &srv->update, 0);
}
...
static void apply_rsp_sending(u16_t duration, int err, void *cb_params)
{
    if (err) {
                           ...
                           struct net_buf_simple *buf)
{
    ...
    LOG_INF(">>> %s from %u (limit: %u)", __func__, idx, limit);
                      ...
                      struct net_buf_simple *buf)
{
    ...
    LOG_INF(">>> %s bt mesh dfu success.", __func__);
...
static inline bool is_active_update(struct bt_mesh_dfu_srv *srv, u8_t idx,
                                    u16_t timeout_base,
                                    const u64_t *blob_id, u8_t ttl,
                                    u16_t meta_checksum)
{
                        ...
                        struct net_buf_simple *buf)
{
    ...
    u16_t timeout_base, meta_checksum;
    enum bt_mesh_dfu_status status;
    u8_t ttl, idx;
    u64_t blob_id;
    int err;
    ...
    LOG_INF(">>> %s %u ttl: %u extra time: %u", __func__, idx, ttl, timeout_base);
    ...
    if (bt_mesh_dfu_srv_is_busy(srv)) {
        ...
        LOG_WRN("Busy. status: %d Phase: %u", status, srv->update.phase);
        goto rsp;
    }
                                ...
                                void *cb_arg)
{
    ...
    err = node_info_load(DFU_SRV_STATE_INDEX, &srv->update, sizeof(srv->update));
    if (err) {
        LOG_ERR("dfu_srv_settings_set load info failed!!");
        return err;
    }
...
static void blob_end(struct bt_mesh_blob_srv *b, u64_t id, bool success)
{
    struct bt_mesh_dfu_srv *srv =
        CONTAINER_OF(b, struct bt_mesh_dfu_srv, blob);

    LOG_DBG(">>> %s success: %u", __func__, success);
...
{
    if (srv->update.phase != BT_MESH_DFU_PHASE_VERIFY) {
        LOG_WRN(">>> %s Wrong state", __func__);
        return;
    }
...
{
    if (srv->update.phase != BT_MESH_DFU_PHASE_APPLYING) {
        LOG_WRN(">>> %s Wrong state: %d", __func__, srv->update.phase);
        return;
    }
...
u8_t bt_mesh_dfu_srv_progress(const struct bt_mesh_dfu_srv *srv)
{
    if (!bt_mesh_dfu_srv_is_busy(srv)) {
        return 0U;
    }
