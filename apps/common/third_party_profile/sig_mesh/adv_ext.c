...
#pragma bss_seg(".ble_mesh_advext_bss")
#pragma data_seg(".ble_mesh_advext_data")
#pragma const_seg(".ble_mesh_advext_const")
#pragma code_seg(".ble_mesh_advext_code")
#endif
                     ...
                     const struct bt_data *sd, size_t sd_len)
{
    ...
    if (atomic_test_bit(ext_adv->flags, ADV_FLAG_UPDATE_PARAMS)) {
        // err = bt_le_ext_adv_update_param(ext_adv->instance, param);//for compiler, not used now.
        if (err) {
    ...
    //err = bt_le_ext_adv_set_data(ext_adv->instance, ad, ad_len, sd, sd_len);//for compiler, not used now.
    if (err) {
    ...
    //err = bt_le_ext_adv_start(ext_adv->instance, start);//for compiler, not used now.
    if (err) {
...
static int bt_data_send(struct bt_mesh_ext_adv *ext_adv, u8_t num_events, u16_t adv_interval,
                        const struct bt_data *ad, size_t ad_len)
{
    struct bt_le_ext_adv_start_param start = {
...
{
    u8_t num_events = BT_MESH_TRANSMIT_COUNT(adv->ctx.xmit) + 1;
...
static void send_pending_adv(struct k_work *work)
{
    ...
    // while ((adv = bt_mesh_adv_get_by_tag(ext_adv->tags, K_NO_WAIT))) { //for compiler, not used now.
...
static bool schedule_send(struct bt_mesh_ext_adv *ext_adv)
{
    if (atomic_test_and_clear_bit(ext_adv->flags, ADV_FLAG_PROXY)) {
        atomic_clear_bit(ext_adv->flags, ADV_FLAG_PROXY_START);
        // (void)bt_le_ext_adv_stop(ext_adv->instance);//for compiler, not used now.
    ...
    if (atomic_test_bit(ext_adv->flags, ADV_FLAG_ACTIVE)) {
        ...
        return false;
        // } else if (k_work_is_pending(&ext_adv->work)) { //for compiler, not used now.
        // 	return false;
    }
...
int bt_mesh_adv_terminate(struct bt_mesh_adv *adv)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(advs); i++) {
        ...
        // err = bt_le_ext_adv_stop(ext_adv->instance);//for compiler, not used now.
        if (err) {
...
int bt_mesh_adv_enable(void)
{
    int err;
    int *adv_cb = NULL;
// 	static const struct bt_le_ext_adv_cb adv_cb = { //for compiler, not used now.
// 		.sent = adv_sent,
// #if defined(CONFIG_BT_MESH_GATT_SERVER)
// 		.connected = connected,
// #endif /* CONFIG_BT_MESH_GATT_SERVER */
...
int bt_mesh_adv_disable(void)
{
    ...
    // struct k_work_sync sync;//for compiler, not used now.
    int *sync = NULL;
    ...
    for (int i = 0; i < ARRAY_SIZE(advs); i++) {
        ...
        // 	k_work_flush(&advs[i].work, &sync);	//for compiler, not used now.
        // }

        // err = bt_le_ext_adv_stop(advs[i].instance);//for compiler, not used now.
        if (err) {
            LOG_ERR("Failed to stop adv %d", err);
            return err;
        }
...
int bt_mesh_adv_bt_data_send(u8_t num_events, u16_t adv_interval,
                             const struct bt_data *ad, size_t ad_len)
{
