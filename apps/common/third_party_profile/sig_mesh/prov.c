...
int bt_mesh_prov_reset_state(void)
{
    ...
    (void)memset((u8_t *)&bt_mesh_prov_link + offset, 0,
...
static int check_output_auth(bt_mesh_output_action_t output, u8_t size)
{
    if (!output) {
        return -EINVAL;
    }
...
static int check_input_auth(bt_mesh_input_action_t input, u8_t size)
{
    if (!input) {
        return -EINVAL;
    }
...
static void get_auth_string(char *str, u8_t size)
{
    uint64_t value;
...
static u32_t get_auth_number(bt_mesh_output_action_t output,
                             bt_mesh_input_action_t input, u8_t size)
{
    const u32_t divider[PROV_IO_OOB_SIZE_MAX] = { 10, 100, 1000, 10000,
