...
#pragma bss_seg(".ble_mesh_dfuslot_bss")
#pragma data_seg(".ble_mesh_dfuslot_data")
#pragma const_seg(".ble_mesh_dfuslot_const")
#pragma code_seg(".ble_mesh_dfuslot_code")
#endif
...
static u32_t slot_index;

static char *slot_entry_encode(u16_t idx, char buf[SLOT_ENTRY_BUFLEN],
                               const char *property)
{
...
{
    u16_t idx = ARRAY_INDEX(slots, slot_to_store);
...
{
    u16_t idx = ARRAY_INDEX(slots, slot_to_erase);
    char buf[SLOT_ENTRY_BUFLEN];
...
int bt_mesh_dfu_slot_info_set(struct bt_mesh_dfu_slot *dfu_slot, size_t size,
                              const u8_t *metadata, size_t metadata_len)
{
...
const struct bt_mesh_dfu_slot *bt_mesh_dfu_slot_at(u16_t img_idx)
{
    struct slot *s;
...
int bt_mesh_dfu_slot_get(const u8_t *fwid, size_t fwid_len, struct bt_mesh_dfu_slot **slot)
{
    struct slot *s;
    int idx = 0;
