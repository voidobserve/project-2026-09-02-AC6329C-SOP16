...
static int bt_mesh_aes_cmac_one_raw_key(const u8_t key[16], const void *m,
...
static int bt_mesh_sha256_hmac_one_raw_key(const u8_t key[32], const void *m, size_t len,
        u8_t mac[32])
{
    struct bt_mesh_sg sg = {.data = m, .len = len};
...
int bt_mesh_k2(const u8_t n[16], const u8_t *p, size_t p_len,
               u8_t net_id[1], struct bt_mesh_key *enc_key, struct bt_mesh_key *priv_key)
{
...
int bt_mesh_k5(const u8_t *n, size_t n_len, const u8_t salt[32],
               u8_t *p, u8_t out[32])
{
    u8_t t[32];
    int err;
...
static void create_proxy_sol_nonce(u8_t nonce[13], const u8_t *pdu)
{
    memset(nonce, 0, 13);
                             ...
                             const struct bt_mesh_app_crypto_ctx *ctx)
{
    ...
    sys_put_be32((ctx->seq_num | ((u32_t)ctx->aszmic << 31)), &nonce[1]);
...
int bt_mesh_prov_salt(u8_t algorithm,
                      const u8_t *conf_salt,
                      const u8_t *prov_rand,
                      const u8_t *dev_rand,
                      u8_t *prov_salt)
{
    u8_t size = algorithm ? 32 : 16;
...
int bt_mesh_prov_conf_salt(u8_t algorithm, const u8_t conf_inputs[145],
                           u8_t *salt)
{
...
int bt_mesh_prov_conf_key(u8_t algorithm, const u8_t *k_input,
                          const u8_t *conf_salt, u8_t *conf_key)
{
    if (algorithm == BT_MESH_PROV_AUTH_HMAC_SHA256_AES_CCM &&
        IS_ENABLED(CONFIG_BT_MESH_ECDH_P256_HMAC_SHA256_AES_CCM)) {
...
int bt_mesh_prov_conf(u8_t algorithm, const u8_t *conf_key,
                      const u8_t *prov_rand, const u8_t *auth, u8_t *conf)
{
    u8_t auth_size = algorithm ? 32 : 16;
...
static int private_beacon_obf(const struct bt_mesh_key *pbk, const u8_t data[5],
                              const u8_t random[13], u8_t out[5])
{
    u8_t salt[16];
    int i, err;
...
static int private_beacon_auth(const struct bt_mesh_key *pbk,
                               const u8_t beacon_data[5],
                               const u8_t random[13], u8_t auth[8])
{
    u8_t salt[16], tmp[16];
    int i, err;
