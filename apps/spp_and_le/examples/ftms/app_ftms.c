...
static u8 is_app_ftms_active = 0;
 ...
 */
/*************************************************************************************************/
void ftms_power_event_to_user(u8 event)
{
    struct sys_event e;
...
static void ftms_set_soft_poweroff(void)
{
    log_info("set_soft_poweroff\n");
    is_app_ftms_active = 1;
...
static void ftms_timer_handle_test(void)
{
    log_info("not_bt");
...
#else
//no bt,to for test
        sys_timer_add(NULL, ftms_timer_handle_test, 1000);
#endif
    }
    /* 按键消息使能 */
 ...
 */
/*************************************************************************************************/
static int ftms_state_machine(struct application *app, enum app_state state, struct intent *it)
{
    switch (state) {
    case APP_STA_CREATE:
        break;
 ...
 */
/*************************************************************************************************/
static int ftms_bt_hci_event_handler(struct bt_event *bt)
{
 ...
 */
/*************************************************************************************************/
static int ftms_bt_connction_status_event_handler(struct bt_event *bt)
{
 ...
 */
/*************************************************************************************************/
static void ftms_key_event_handler(struct sys_event *event)
{
    /* u16 cpi = 0; */
    u8 event_type = 0;
    u8 key_value = 0;
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if (event_type == KEY_EVENT_TRIPLE_CLICK
            ...
            //for test
            ftms_power_event_to_user(POWER_EVENT_POWER_SOFTOFF);
            return;
        }
 ...
 */
/*************************************************************************************************/
static int ftms_event_handler(struct application *app, struct sys_event *event)
{
    switch (event->type) {
    case SYS_KEY_EVENT:
        ftms_key_event_handler(event);
        return 0;

    case SYS_BT_EVENT:
#if (TCFG_USER_EDR_ENABLE || TCFG_USER_BLE_ENABLE)
        if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
            ftms_bt_connction_status_event_handler(&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
            ftms_bt_hci_event_handler(&event->u.bt);
        }
#endif
        return 0;
    ...
    case SYS_DEVICE_EVENT:
        if ((u32)event->arg == DEVICE_EVENT_FROM_POWER) {
            return app_power_event_handler(&event->u.dev, ftms_set_soft_poweroff);
        }
...
static const struct application_operation app_ftms_ops = {
    .state_machine  = ftms_state_machine,
    .event_handler 	= ftms_event_handler,
};
 ...
 */
/*************************************************************************************************/
static u8 ftms_state_idle_query(void)
{
    return !is_app_ftms_active;
}

REGISTER_LP_TARGET(ftms_state_lp_target) = {
    .name = "ftms_state_deal",
    .is_idle = ftms_state_idle_query,
};
